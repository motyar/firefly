	...
	$.firefly.create = function(img){
		...
		$(document.body).append(spark);
			return spark.css({
				...
				top: $.firefly.random(($(window).height()-$.firefly.offset($(window).height() ))),	//offsets
				left: $.firefly.random(($(window).width()-$.firefly.offset($(window).width() )))		//offsets
				}).show();
}
...
$.firefly.fly = function(sp) {
	...
	$(sp).animate({
		top: $.firefly.random(($(window).height()-$.firefly.offset( $(window).height() ))),	//offsets
		left: $.firefly.random(($(window).width()-$.firefly.offset( $(window).width() )))
	},
	
	(($.firefly.random(10) + 5) * 1100 ),
		
	function(){ $.firefly.fly(sp) } );
};
...
$.forefly.op(opc)
{
	return Math.random()*opc;
}
$.firefly.offset(off)
{
	return $.firefly.random(off/2);
}
})(jQuery);
